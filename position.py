# Auteurs: ARISTOPHANE MAFA & HenryGarrafa
...
class Position:
    ...
    def positions_diagonales_bas(self):
        ...
        """
        diagonale_haut_gauche = Position(self.ligne + 1, self.colonne - 1)
        diagonale_haut_droite = Position(self.ligne + 1, self.colonne + 1)
        return [diagonale_haut_gauche, diagonale_haut_droite]
    ...
    def positions_diagonales_haut(self):
        ...
        """
        # Calcul des positions diagonales supérieures
        diagonale_haut_gauche = Position(self.ligne - 1, self.colonne - 1)
        diagonale_haut_droite = Position(self.ligne - 1, self.colonne + 1)

        # Retourne la liste des positions diagonales supérieures
        return [diagonale_haut_gauche, diagonale_haut_droite]
    ...
    def quatre_positions_diagonales(self):
        ...
        """
        # Obtenir les positions diagonales supérieures et inférieures
        diagonales_haut = self.positions_diagonales_haut()
        diagonales_bas = self.positions_diagonales_bas()

        # Retourner la concaténation des deux listes de positions diagonales
        return diagonales_haut + diagonales_bas
    ...
    def quatre_positions_sauts(self):
        ...
        """
        # Calcul des positions de "sauts" diagonaux à distance 2
        saut_haut_gauche = Position(self.ligne + 2, self.colonne - 2)
        saut_haut_droite = Position(self.ligne + 2, self.colonne + 2)
        saut_bas_gauche = Position(self.ligne - 2, self.colonne - 2)
        saut_bas_droite = Position(self.ligne - 2, self.colonne + 2)

        # Retourne la liste des positions de "sauts" diagonaux
        return [saut_haut_gauche, saut_haut_droite, saut_bas_gauche, saut_bas_droite]
...
if __name__ == '__main__':
    ...
    assert repr(pos1) == '(2, 3)'

    # Test des positions diagonales
    assert pos1.positions_diagonales_bas() == [Position(3, 2), Position(3, 4)]
    assert pos1.positions_diagonales_haut() == [Position(1, 2), Position(1, 4)]
    assert pos1.quatre_positions_diagonales() == [Position(1, 2), Position(1, 4), Position(3, 2), Position(3, 4)]

    # Test des positions de sauts
    assert pos1.quatre_positions_sauts() == [Position(4, 1), Position(4, 5), Position(0, 1), Position(0, 5)]
